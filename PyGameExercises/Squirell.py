...
WINWIDTH = 960# width of the program's window, in pixels #old win width was 640 , so we change that to 960
...
CAMSLACKNEW1 = 75 # NEW VALUES FOR CAMERA SLACK HORIZONTAL
CAMSLACKNEW2 = 75 # NEW VALUES FOR CAMERA SLACK VERTICAL
...
LOSTSIZE = 20        # LOST SIZE , THE SIZE WHERE WE LOSE THE GAME IF WE GET TOUCHET BY SQUIRELLS
...
def runGame():
    ...
    while True: # main game loop
        ...
        if (camerax + HALF_WINWIDTH) - playerCenterx > CAMSLACKNEW1:
            camerax = playerCenterx + CAMSLACKNEW1 - HALF_WINWIDTH
        elif playerCenterx - (camerax + HALF_WINWIDTH) > CAMSLACKNEW1:
            camerax = playerCenterx - CAMSLACKNEW1 - HALF_WINWIDTH
        if (cameray + HALF_WINHEIGHT) - playerCentery > CAMSLACKNEW2:
            cameray = playerCentery + CAMSLACKNEW2 - HALF_WINHEIGHT
        elif playerCentery - (cameray + HALF_WINHEIGHT) > CAMSLACKNEW2:
            cameray = playerCentery - CAMSLACKNEW2 - HALF_WINHEIGHT
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                       ...
                       # here we define the new logic of losing the size of the squirell
                        playerObj['size'] -= int((sqObj['width'] * sqObj['height']) ** 0.2) + 1
                       #here we define the condition of the size , if the size of the object is smaller or equal to the LOSTSIZE , we terminate the game
